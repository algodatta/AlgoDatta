...
async def tradingview(request: Request, db: Session = Depends(get_db)):
    ...
    secret = body.get("secret"); strategy_id = body.get("strategy_id") or body.get("strategyId"); side = body.get("side"); qty = int(body.get("qty", 1)); price = body.get("price"); idemp = body.get("idempotency_key")
    if not all([secret, strategy_id, side, idemp]): raise HTTPException(400, "Missing required fields")
    s = db.query(Strategy).filter(Strategy.id == int(strategy_id)).first()
    if not s or s.webhook_secret != secret: raise HTTPException(401, "Invalid secret or strategy")
