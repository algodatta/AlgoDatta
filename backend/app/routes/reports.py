...
def export_csv(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    f = StringIO(); writer = csv.writer(f)
    writer.writerow(["id","strategy_id","side","qty","price","status","created_at"])
    for r in rows: writer.writerow([r.id, r.strategy_id, r.side, r.qty, r.price, r.status, (r.created_at.isoformat() if r.created_at else "")])
