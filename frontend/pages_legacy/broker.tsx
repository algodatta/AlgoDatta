...
export default function BrokerPage(){
  ...
  const link = async () => {
    try{
      ...
      await api.post('/api/broker', { broker_type: broker, client_id: clientId, access_token: accessToken, auth_token: accessToken }, { headers: authHeaders() });
  ...
  return (<div className="min-h-screen bg-gray-100 p-6">
    <div className="max-w-xl mx-auto bg-white rounded-xl shadow p-4 space-y-3">
      ...
      <select className="border rounded p-2 w-full" value={broker} onChange={e=>setBroker(e.target.value)}><option value="dhan">Dhan</option></select>
